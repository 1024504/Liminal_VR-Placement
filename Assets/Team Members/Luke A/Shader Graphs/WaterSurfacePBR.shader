Shader "WaterSurfacePBR"
{
	...
	SubShader
    {
		...
		{
            float3 normalScroll = lerp(UnpackNormal(tex2D(_NormalTex, i.uv_NormalTex+_NormalSpeed1*_Time[1])), UnpackNormal(tex2D(_NormalTex, i.uv_NormalTex+_NormalSpeed2*_Time[1])).rgb, 0.5);
            normalScroll.b *= -1;
